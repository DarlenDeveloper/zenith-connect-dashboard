...
const AIRIESLogo: React.FC<AIRIESLogoProps> = ({
  size = 40,
  color = 'currentColor',
  className = '',
  showText = false,
  textClassName = 'text-white font-bold ml-2',
  animate = false,
}) => {
  // Calculate size in pixels if a number is provided
    ...
    <div className={`flex items-center ${className}`}>
      ...
      >
        ...
        >
          ...
          <g clipPath="url(#roundedCorners)" className={`fill-${color}`}>
      ...
      {showText && <span className={textClassName}>AIRIES</span>}
    </div>
  );
};

export default AIRIESLogo; 